...
public class DList <T> {
	...
	private InvocationHandler handler = new InvocationHandler() {
		...
		public Object invoke(Object proxy, Method method, Object[] args)
				throws Throwable {
			ObjectLocation objectLocation = dListCore.getObjectLocation(System.identityHashCode(proxy));
			return messageSender.sendMethodInvocationMessage(objectLocation, method, args);
		}
	};
	...
	public void add(T object) {
		...
		messageSender.sendAdditionIntoRemoteListMessage(DSpaceController.getOwner(),
				dListCore.getRemoteListOwners(), dListCore.getSpace(),
				dListCore.getListName(), -1);
	}
	...
	public void add(String owner, int index, T object) {
		...
		messageSender.sendAdditionIntoRemoteListMessage(owner,
				receivers, dListCore.getSpace(),
				dListCore.getListName(), index);
	}
	
	public T remove(int index) { // better use local
		T object = dListCore.removeObject(index);
		messageSender.sendRemovalFromRemoteListMessage(DSpaceController.getOwner(),
				dListCore.getRemoteListOwners(), dListCore.getSpace(),
				dListCore.getListName(), index);
		return object;
	}
	...
	/*public boolean remove(String owner, Object object) {
		int index = messageSender
				.sendRemovalFromLocalListByObjectMessage(owner,
						dListCore.getSpace(), dListCore.getListName(), object);
		if (index >= 0) {
			dListCore.removeProxyObject(owner, index);
			List<String> receivers = dListCore.getRemoteListOwners();
			receivers.remove(owner);
